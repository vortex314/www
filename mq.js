...
mqttCtrl = mqttApp.controller('mqttCtrl', function ($scope) {
    ...
    $scope.getTopic = function (str) {
        if (typeof $scope.topics[str] === "undefined") return "UNDEF"
        else
...
mqttCtrl.directive('deltaTime', function () {
    return {
        restrict: 'C',
        template: '<p class="bg-primary">{{topicObject.name}}</p><h1 class="bg-primary">{{strTime()}}</h1>',
        scope: {
            topicObject: '=topic'
        },
        ...
        link: function (scope, element, attrs) {
            scope.topic3 = scope.$parent.getTopic(attrs.topic);
        }
    }
});
...
mqttCtrl.directive('absTime', function () {
    return {
        restrict: 'C',
        template: '<div><h4>Topic --<kbd>{{strTime()}}</kbd></h4></div>',
        scope: {
            topicObject: '=topic'
        },
        controller: function ($scope) {
...
mqttCtrl.directive('lmrG', function () {
 return {
        restrict: 'C',
        template: '<div style="width:400; height:300"></div>',
//        templateUrl: 'lmr-gauge.html',
        scope: {
            topicObject: '=topic'
        },
    link : function (scope, element, attrs) {
        scope.$watch(attrs.topic, function () {
            console.log(attrs.topic);
        });
    }
    }
});
